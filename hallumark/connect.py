"""Native cognis-connect emit for hallumark — forward findings to any platform.

Maps hallumark's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    hallumark ... --format json | hallumark-emit --to stix
    hallumark-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('question', '')
        out['severity'] = 'medium'
        out['type'] = 'hallucination'
        out['description'] = rec.get('answer', '')
