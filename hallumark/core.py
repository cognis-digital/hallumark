...
TOOL_NAME = "HALLUMARK"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'HM-GROUND-001', 'severity': 'high', 'weight': 2.5, 'title': 'UNGROUNDED_CLAIM', 'when': {'grounded': False}, 'remediation': 'Reject ungrounded answers / lower temperature / improve retrieval.'}, {'id': 'HM-CITE-001', 'severity': 'medium', 'weight': 2.0, 'title': 'BAD_CITATION', 'when': {'citation_valid': False}, 'remediation': 'Validate citations against retrieved chunks.'}, {'id': 'HM-CONFLICT-001', 'severity': 'high', 'weight': 2.5, 'title': 'CONTRADICTS_CONTEXT', 'when': {'contradicts_context': True}, 'remediation': 'Use a contradiction detector before returning answer.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="rag-grounding",
                    metadata=item,
                ))
